          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="text-sm text-gray-400 bg-gray-700/30 p-2 rounded">
                Receita: {recipes.COLETE.aluminum} Alumínio, {recipes.COLETE.rubber} Borracha, {recipes.COLETE.clothes} Roupas
              </div>
            ...
            <div className="space-y-3">
              ...
              />
              <div className="text-sm text-gray-400 bg-gray-700/30 p-2 rounded">
                Receita: {recipes.TROJAN.aluminum} Alumínio, {recipes.TROJAN.rubber} Borracha
              </div>
              <div className="flex space-x-2">
                <Badge variant="secondary" className="bg-green-600/20 text-green-400 border-green-600/30">
            ...
            <div className="space-y-3">
              ...
              />
              <div className="text-sm text-gray-400 bg-gray-700/30 p-2 rounded">
                Receita: {recipes.NOTEBOOK.aluminum} Alumínio, {recipes.NOTEBOOK.rubber} Borracha
              </div>
              <div className="flex space-x-2">
                <Badge variant="secondary" className="bg-green-600/20 text-green-400 border-green-600/30">
            ...
            <div className="space-y-3">
              ...
              />
              <div className="text-sm text-gray-400 bg-gray-700/30 p-2 rounded">
                Receita: {recipes.CELULAR_DESCARTAVEL.aluminum} Alumínio, {recipes.CELULAR_DESCARTAVEL.rubber} Borracha, {recipes.CELULAR_DESCARTAVEL.clothes} Roupas
              </div>
              <div className="flex space-x-2">
                <Badge variant="secondary" className="bg-green-600/20 text-green-400 border-green-600/30">
                ...
                <Badge variant="secondary" className="bg-red-600/20 text-red-400 border-red-600/30">
                  Não-aliado: {formatCurrency(pricing.CELULAR_DESCARTAVEL.nonAlly)}
                </Badge>
              </div>
            </div>
          </CardContent>
              ...
              <div className="text-sm text-gray-300 space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {coleteQuantity + trojanQuantity + notebookQuantity + celularQuantity}
                  </span>
                </div>
                <div className="flex justify-between">
        ...
        <Card className="bg-gray-800/50 border-gray-700 backdrop-blur-sm">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="donatedClothes" className="text-white font-medium">
              ...
              <Input
                ...
                onChange={(e) =>
                  setDonatedClothes(Math.max(0, Number.parseInt(e.target.value) || 0))
                }
                placeholder="0"
                className="text-lg bg-gray-700/50 border-gray-600 text-white placeholder:text-gray-400 focus:border-blue-500 focus:ring-blue-500"
              />
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-gradient-to-r from-green-600/20 to-emerald-500/20 rounded-lg border border-green-500/30">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    <span className="text-xl">🤝</span>
                    <span className="font-semibold text-green-300">Preço Aliado</span>
                  </div>
                ...
                {(coleteQuantity > 0 || trojanQuantity > 0 || notebookQuantity > 0 || celularQuantity > 0) && (
                  <div className="text-sm text-green-300/80 mt-2 space-y-1">
                    {coleteQuantity > 0 && (
                      <div>
                        COLETE: {coleteQuantity} × {formatCurrency(pricing.COLETE.ally)} ={' '}
                        {formatCurrency(coleteQuantity * pricing.COLETE.ally)}
                      </div>
                    )}
                    {trojanQuantity > 0 && (
                      <div>
                        TROJAN: {trojanQuantity} × {formatCurrency(pricing.TROJAN.ally)} ={' '}
                        {formatCurrency(trojanQuantity * pricing.TROJAN.ally)}
                      </div>
                    )}
                    {notebookQuantity > 0 && (
                      <div>
                        NOTEBOOK: {notebookQuantity} × {formatCurrency(pricing.NOTEBOOK.ally)} ={' '}
                        {formatCurrency(notebookQuantity * pricing.NOTEBOOK.ally)}
                      </div>
                    )}
                    {celularQuantity > 0 && (
                      <div>
                        CELULAR DESC.: {celularQuantity} × {formatCurrency(pricing.CELULAR_DESCARTAVEL.ally)} ={' '}
                        {formatCurrency(celularQuantity * pricing.CELULAR_DESCARTAVEL.ally)}
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="p-4 bg-gradient-to-r from-red-600/20 to-orange-500/20 rounded-lg border border-red-500/30">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-semibold text-red-300">Preço Não-Aliado</span>
                  </div>
                ...
                {(coleteQuantity > 0 || trojanQuantity > 0 || notebookQuantity > 0 || celularQuantity > 0) && (
                  <div className="text-sm text-red-300/80 mt-2 space-y-1">
                    {coleteQuantity > 0 && (
                      <div>
                        COLETE: {coleteQuantity} × {formatCurrency(pricing.COLETE.nonAlly)} ={' '}
                        {formatCurrency(coleteQuantity * pricing.COLETE.nonAlly)}
                      </div>
                    )}
                    {trojanQuantity > 0 && (
                      <div>
                        TROJAN: {trojanQuantity} × {formatCurrency(pricing.TROJAN.nonAlly)} ={' '}
                        {formatCurrency(trojanQuantity * pricing.TROJAN.nonAlly)}
                      </div>
                    )}
                    {notebookQuantity > 0 && (
                      <div>
                        NOTEBOOK: {notebookQuantity} × {formatCurrency(pricing.NOTEBOOK.nonAlly)} ={' '}
                        {formatCurrency(notebookQuantity * pricing.NOTEBOOK.nonAlly)}
                      </div>
                    )}
                    {celularQuantity > 0 && (
                      <div>
                        CELULAR DESC.: {celularQuantity} × {formatCurrency(pricing.CELULAR_DESCARTAVEL.nonAlly)} ={' '}
                        {formatCurrency(celularQuantity * pricing.CELULAR_DESCARTAVEL.nonAlly)}
                      </div>
                    )}
                  </div>
                )}
              ...
              {allyTotal > 0 && nonAllyTotal > 0 && (
                <div className="p-3 bg-gradient-to-r from-yellow-600/20 to-amber-500/20 rounded-lg border border-yellow-500/30">
                  <div className="flex items-center space-x-2 mb-1">
                    <span className="text-lg">💡</span>
                    <span className="font-medium text-yellow-300">Economia</span>
                  </div>
                  <div className="text-lg font-bold text-yellow-400">
                    {formatCurrency(nonAllyTotalWithDiscount - allyTotalWithDiscount)}
                  </div>
